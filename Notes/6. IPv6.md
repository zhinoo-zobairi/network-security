...
A **unicast address** identifies a single interface. Packets sent to a unicast go to exactly **one device**. Like speaking directly to one person in a one-to-one conversation.
...
2. **Link-Local Unicast**  
   ...
   - **Real-life:**  
     ...
     - Example: When your laptop boots(starts up), it automatically assigns itself a link-local address. It uses this to ask: “Hey, who is the default router?” (This “asking” happens with a special kind of message called **ICMPv6 Neighbor Discovery**):
      ...
      3. Router replies from its own **Link-Local** (e.g. `fe80::1`), saying: *“I am your default gateway, and here’s the global prefix you should use.”* (IPv6 rules require that router discovery and neighbor discovery always use **link-local** addresses. This guarantees they only operate inside the link where they make sense and don’t leak into the wider Internet.)
...
A **multicast address** identifies a group of interfaces. Packets sent to a multicast are delivered to **all members of the group**. (⚠️ IPv6 has **no broadcast at all**). You “speak” to a group that understands a specific dialect.

- **Prefix:** `ff00::/8`  
- **Scope:** Defined by flags, common ones include:  
  - `ff02::1` → All nodes on the local link – is the “all-nodes dialect”—every IPv6-enabled device listens.
  - `ff02::2` → All routers on the local link – is the “router dialect”—only routers respond
- **Real-life:**  
...
An Anycast address can be used by **multiple machines** at the same time. But when you send a packet to that address, only **one machine** receives it, the one that is **closest** according to the routing system. This is different from multicast (many receivers) and unicast (exactly one fixed receiver). Like speaking a shared language but only the nearest speaker that hears it, answers.
...
4. **Global Unicast Delegation (`2000::/3`)**  
   ...
   - The laptop constructs a **Global Unicast address** by appending a host identifier.  
    ...
    - Stateless Address Autoconfiguration (SLAAC) builds on top of that foundation. The router tells the host, “Here’s the prefix,” and the host configures a global unicast address.
